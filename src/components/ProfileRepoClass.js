...
class ProfileRepoClass extends Component {
  ...
  async componentDidMount() {
    const data = await fetch( Github_UserApi + Github_UserName + "/repos", options
    );
  ...
  render() {
    ...
    return (
      <div className="pro-repository-container">
        {repoList
          .filter(
            (repo) => repo.name === Github_repoName).map(
                (repo) => {
                  return (
                    <div key={repo.id}>
                        <h1>
                        ...
                        <h3 className="repo-descr">{repo.description}</h3>
